export const tr = {
  appName: 'Marketiyo',

  navDashboard: 'Genel bakış',
  onboardingLockedHint: 'Önce kurulum adımlarını tamamla',
  onboardingTitle: 'Başlamadan önce',
  onboardingIntro: 'Bu iki adımı sırayla tamamla. İkisi bitmeden fikir, takvim, reklam ve blog açılmıyor.',
  onboardingStep: 'Adım',
  onboardingDone: 'Tamamlandı',
  onboardingLocked: 'Önceki adımı bitir',
  onboardingStart: 'Başla',
  onboardingSetupTitle: 'İçeriği hangi modelin üreteceğini seç',
  onboardingSetupBody: 'Claude Code aboneliğini ya da kendi Gemini anahtarını bağla. Bağlantı sınanıp çalıştığı doğrulanınca bu adım tamamlanır.',
  onboardingBrandTitle: 'Marka profilini oluştur',
  onboardingBrandBody: 'Ürünün ne olduğunu, kime seslendiğini ve elindeki kanıtları yaz. Üretilen her şey buna dayanıyor.',
  onboardingAllDone: 'Kurulum tamam, her şey açık.',
  setupNotVerified: 'Bağlantı doğrulanamadığı için kurulum tamamlanmadı. Sorunu giderip tekrar dene.',
  setupNextBrand: 'Sonraki adım: marka profili',
  brandNextDashboard: 'Kurulum tamam, panoya git',

  dashGreeting: 'Genel bakış',
  dashNoBrand: 'Henüz marka profili yok',
  dashNextStep: 'Sıradaki adım',
  dashGo: 'Git',

  nextSetupTitle: 'Önce içeriği hangi modelin üreteceğini seç',
  nextSetupBody: 'Claude Code aboneliğini ya da kendi Gemini anahtarını bağla. Bir dakika sürer.',
  nextBrandTitle: 'Önce markanı tanıt',
  nextBrandBody: 'Üretilen her şey marka profiline dayanıyor. Birkaç dakikanı ayır, gerisi çok daha iyi çıkar.',
  nextPostTodayTitle: 'Bugün paylaşılacak içerik var',
  nextPostTodayBody: 'Takvimde bugüne yerleştirdiğin içeriği paylaş, sonra paylaşıldı olarak işaretle.',
  nextScheduleTitle: 'Hazır içerik takvime yerleştirilmeyi bekliyor',
  nextScheduleBody: 'Metni üretilmiş ama henüz bir güne konmamış içeriğin var.',
  nextExpandTitle: 'Sakladığın fikirleri içeriğe dönüştür',
  nextExpandBody: 'Beğenip sakladığın fikirlerin henüz senaryosu veya metni yok.',
  nextTriageTitle: 'Yeni fikirleri gözden geçir',
  nextTriageBody: 'Beğendiklerini sakla, beğenmediklerini nedeniyle ele. Eleme nedeni bir sonraki partiyi değiştiriyor.',
  nextGenerateTitle: 'Yeni fikir üret',
  nextGenerateBody: 'Bekleyen iş yok. Yeni bir parti fikir üretme zamanı.',

  dashToday: 'Bugün',
  dashTodayEmpty: 'Bugüne yerleştirilmiş içerik yok.',
  dashThisWeek: 'Bu hafta',
  dashScheduled: 'planlandı',
  dashPosted: 'paylaşıldı',
  dashPipeline: 'Fikir hattı',
  dashPipelineNew: 'Gözden geçirilecek',
  dashPipelineKept: 'Saklandı',
  dashPipelineExpanded: 'İçeriği hazır',
  dashPipelineScheduled: 'Takvimde',
  dashAds: 'Reklam',
  dashAdsRunning: 'şu an yayında',
  dashAdsTotal: 'toplam kampanya',
  dashBlog: 'Blog',
  dashBlogKept: 'saklanan konu',
  dashBlogDrafted: 'taslağı hazır',
  dashOpenCalendar: 'Takvimi aç',
  dashOpenIdeas: 'Fikirlere git',
  dashOpenAds: 'Kampanyalara git',
  dashOpenBlog: 'Bloga git',
  navIdeas: 'Fikirler',
  navCalendar: 'Takvim',
  navBrand: 'Marka',
  navSetup: 'Kurulum',
  navGuide: 'Rehber',
  guideTitle: 'Marketiyo nasıl çalışıyor',
  guideIntro: 'Amaç, yöntem ve yapay zekâ bağlantısının tam olarak nasıl kurulduğu.',
  guideContents: 'İçindekiler',

  setupTitle: 'Kurulum',
  setupEngine: 'İçeriği hangi model üretsin?',
  setupClaudeCode: 'Claude Code, mevcut aboneliğinle',
  setupClaudeCodeHint:
    'Bilgisayarında Claude Code kurulu ve giriş yapılmış olmalı. API anahtarı gerekmez.',
  setupGemini: 'Gemini, kendi API anahtarınla',
  setupGeminiHint:
    'Anahtar yalnızca bu bilgisayarda saklanır ve sadece Google sunucularına gönderilir.',
  setupGeminiModel: 'Model',
  setupGeminiModelHint:
    'Ücretsiz anahtarda Flash Lite modelleri günde yaklaşık 500, Flash modelleri yalnızca 20 istek hakkı verir. Güncel sınırların Google AI Studio’da görünür.',
  setupGeminiModelRecommended: 'önerilen',
  setupSave: 'Kaydet ve bağlantıyı sına',
  setupChecking: 'Bağlantı sınanıyor',
  setupAvailable: 'Bağlantı kuruldu',
  setupUnavailable: 'Bağlantı kurulamadı',

  brandTitle: 'Marka profili',
  brandIntro:
    'Üretilen her fikir bu sayfadaki bilgilere dayanır. Ne kadar somut yazarsan, fikirler o kadar sana özel olur.',

  brandDraftLabel: 'Ürününü anlat',
  brandDraftHint:
    'Ürününü tarif eden bir metni buraya yapıştır, aşağıdaki alanları senin için dolduralım. Sonra düzeltirsin.',
  brandDraftAction: 'Taslağı oluştur',
  brandDrafting: 'Taslak çıkarılıyor, yarım dakika kadar sürebilir',

  brandBasics: 'Temel bilgiler',
  brandProductName: 'Ürün adı',
  brandOneLiner: 'Ürün tek cümleyle ne yapıyor?',
  brandCategory: 'Hangi kategoride? Örneğin geliştirici aracı, kafe, ajans',

  brandAudiences: 'Kime sesleniyorsun?',
  brandAudiencesHint: 'Her kitle için ayrı bir satır ekle.',
  brandAudienceLabel: 'Kitleyi tek kelimeyle tanımla',
  brandAudiencePain: 'Şu an yaşadığı sıkıntı',
  brandAudienceDesire: 'Ulaşmak istediği sonuç',
  brandAudienceWhere: 'Vaktini nerede geçiriyor?',

  brandVoiceDo: 'Üslubunda olmasını istediklerin',
  brandVoiceDoHint: 'Örnek: sade konuş, ekrandan göster, rakam ver.',
  brandVoiceDont: 'Üslubunda asla istemediklerin',
  brandVoiceDontHint: 'Örnek: abartılı vaat verme, klişe motivasyon cümlesi kurma.',

  brandProof: 'Elindeki somut kanıtlar',
  brandProofHint:
    'Gerçekten söyleyebileceğin rakamlar, sonuçlar veya gösterebileceğin şeyler. Model burada olmayan bir sayıyı uydurmaz.',

  brandBannedClaims: 'Asla kullanılmayacak ifadeler',
  brandBannedClaimsHint: 'Yasal ya da marka açısından söylemek istemediğin iddialar.',

  brandOutputLanguage: 'İçerikler hangi dilde üretilsin?',
  brandPlatforms: 'Hangi medya türlerini üretsin?',
  brandSave: 'Marka profilini kaydet',
  brandSaving: 'Kaydediliyor',
  brandSaved: 'Kaydedildi.',

  ideasTitle: 'Fikir bankası',
  ideasIntro: 'Beğendiklerini sakla, beğenmediklerini nedeniyle ele. Bir sonraki parti buna göre değişir.',
  ideasPlatform: 'Medya türü',
  ideasCount: 'Kaç fikir?',
  ideasGenerate: 'Fikirleri üret',
  ideasGenerating: 'Üretiliyor, yaklaşık yarım dakika sürer',
  ideasGenerated: 'fikir eklendi',
  ideasDiscarded: 'fikir hatalı geldiği için atlandı',
  ideasKeep: 'Sakla',
  ideasReject: 'Ele',
  ideasRejectWhy: 'Neden elendi?',
  ideasExpand: 'İçeriğe dönüştür',
  ideasEmpty: 'Burada gösterilecek fikir yok.',
  ideasFilterPlatform: 'Medya türü',
  ideasFilterStatus: 'Durum',
  ideasFilterAll: 'Tümü',

  statusNew: 'Yeni',
  statusKept: 'Saklandı',
  statusRejected: 'Elendi',
  statusExpanded: 'İçeriği hazır',
  statusScheduled: 'Takvimde',

  reasonOffBrand: 'Markaya uymuyor',
  reasonTooGeneric: 'Fazla genel',
  reasonAlreadyDone: 'Bunu zaten yaptım',
  reasonWrongAudience: 'Yanlış kitleye sesleniyor',
  reasonWeakHook: 'Kancası zayıf',
  reasonOther: 'Başka bir nedenle',

  expansionTitle: 'İçerik',
  expansionGenerate: 'İçeriği üret',
  expansionRegenerate: 'Yeniden üret',
  expansionSave: 'Değişiklikleri kaydet',
  expansionEmpty: 'Bu fikir henüz içeriğe dönüştürülmedi.',

  calendarTitle: 'Paylaşım takvimi',
  calendarIntro: 'Hiçbir şey otomatik paylaşılmaz. Paylaştığında slota kendin işaretlersin.',
  calendarWorking: 'Uygulanıyor',
  calendarPrevWeek: 'Önceki hafta',
  calendarNextWeek: 'Sonraki hafta',
  calendarToday: 'Bugün',
  calendarThisWeek: 'Bu haftaya dön',
  calendarReadyCount: 'hazır içerik bekliyor',
  calendarPlace: '+ İçerik yerleştir',
  calendarNothingReady: 'Hazır içerik yok',
  calendarPickFor: 'Şu güne yerleştir:',
  calendarCancel: 'Vazgeç',
  calendarNotExpanded: 'metni henüz üretilmedi',
  calendarPosted: 'Paylaşıldı',
  calendarMoveEarlier: 'Bir gün geri al',
  calendarMoveLater: 'Bir gün ileri al',
  calendarOpenContent: 'İçeriği aç',
  calendarRemove: 'Takvimden çıkar',
  calendarMissingIdea: 'Bu içerik silinmiş',



  mediaShortVideo: 'Kısa video',
  mediaShortVideoHint: 'Reels, TikTok, YouTube Shorts',
  mediaVisualPost: 'Görsel gönderi',
  mediaVisualPostHint: 'Tek görsel veya karusel · Instagram, LinkedIn, Facebook',
  mediaShortText: 'Kısa metin',
  mediaShortTextHint: 'Tek gönderi veya dizi · X, Threads',
  mediaLongText: 'Uzun metin',
  mediaLongTextHint: 'Deneyim ve hikâye yazısı · LinkedIn, Facebook',
  addRow: 'Satır ekle',
  removeRow: 'Kaldır',
  errorGeneric: 'Bir şeyler ters gitti.',
  resetTitle: 'Tümünü sıfırla',
  resetIntro: 'Uygulamayı ilk açıldığı haline döndürür.',
  resetOpen: 'Sıfırlama seçeneklerini göster',
  resetWillDelete: 'Şunların hepsi kalıcı olarak silinecek:',
  resetItemBrand: 'Marka profili',
  resetItemContent: 'Tüm fikirler, üretilmiş içerikler ve blog konuları',
  resetItemPlans: 'Takvim ve reklam kampanyaları',
  resetItemSettings: 'Ayarlar, motor seçimi ve varsa Gemini anahtarı',
  resetIrreversible: 'Bu işlem geri alınamaz.',
  resetConfirm: 'Evet, hepsini kalıcı olarak sil',
  resetRunning: 'Siliniyor',
  backToIdeas: 'Fikirlere dön',

  navAds: 'Reklam',

  adsTitle: 'Reklam kampanyaları',
  adsIntro:
    'Bütçeyi sen belirlersin. Model yalnızca girdiğin toplamı reklam setleri arasında dağıtır ve gerekçesini yazar.',
  adsNewCampaign: 'Yeni kampanya',
  adsName: 'Kampanya adı',
  adsObjective: 'Hedefin ne?',
  adsAudience: 'Hangi kitleye?',
  adsNetworks: 'Hangi ağlarda yayınlanacak?',
  adsBudget: 'Toplam bütçe',
  adsCurrency: 'Para birimi',
  adsPeriod: 'Kaç gün sürecek?',
  adsNotes: 'Eklemek istediğin bir şey var mı?',
  adsNotesHint: 'Kampanya, dönem veya ürünle ilgili modelin bilmesi gereken her şey.',
  adsGenerate: 'Planı üret',
  adsGenerating: 'Plan kuruluyor, birkaç dakika sürebilir',
  adsDiscarded: 'reklam seti hatalı geldiği için atlandı',
  adsEmpty: 'Henüz kampanya yok.',
  adsOpen: 'Planı aç',
  adsDelete: 'Sil',
  adsDeleteConfirm: 'Bu kampanya silinsin mi?',
  adsPositioning: 'Konumlandırma',
  adsSetBudget: 'Bütçe',
  adsHypothesis: 'Hipotez',
  adsTargeting: 'Hedefleme',
  adsPrimaryText: 'Birincil metin',
  adsHeadlines: 'Başlıklar',
  adsDescriptions: 'Açıklamalar',
  adsVisual: 'Görsel yönü',
  adsKeywords: 'Anahtar kelimeler',
  adsAdText: 'Reklam metni',
  adsHook: 'Kanca',
  adsScript: 'Senaryo',
  adsCopy: 'Kopyala',
  adsCopied: 'Kopyalandı',
  adsBackToList: 'Kampanyalara dön',
  adsTotalBudgetNote: 'Bu tutar senin girdiğin toplamdan dağıtıldı.',

  objectiveAwareness: 'Bilinirlik',
  objectiveTraffic: 'Trafik',
  objectiveLeads: 'Potansiyel müşteri',
  objectiveSales: 'Satış',
  objectiveAppInstalls: 'Uygulama yüklemesi',

  networkMeta: 'Meta, Facebook ve Instagram',
  networkGoogleSearch: 'Google Arama',
  networkTiktok: 'TikTok',

  navSeo: 'Blog',

  seoTitle: 'Arama içeriği',
  seoIntro:
    'Ürününün gerçekten otorite kurabileceği yazı konuları. Arama hacmi veya zorluk puanı üretilmez, o veriye erişim yok.',
  seoFormTitle: 'Yeni konular üret',
  seoCountSuffix: 'adet yazı konusu üret',
  seoRepeatNote: 'Daha önce ürettiğin başlıklar tekrar edilmez.',
  seoCount: 'Adet',
  seoGenerate: 'Konuları üret',
  seoGenerating: 'Konular çıkarılıyor',
  seoGenerated: 'konu eklendi',
  seoDiscarded: 'konu hatalı geldiği için atlandı',
  seoEmpty: 'Burada gösterilecek konu yok.',
  seoKeep: 'Sakla',
  seoReject: 'Ele',
  seoOpen: 'Taslağa geç',
  seoPrimaryKeyword: 'Ana ifade',
  seoSecondaryKeywords: 'Yan ifadeler',
  seoOutline: 'Ana hat',
  seoDraftTitle: 'Yazı taslağı',
  seoDraftGenerate: 'Taslağı üret',
  seoDraftRegenerate: 'Yeniden üret',
  seoDraftSave: 'Değişiklikleri kaydet',
  seoDraftEmpty: 'Bu konu henüz taslağa dönüştürülmedi.',
  seoBackToList: 'Konulara dön',

  intentInformational: 'Bilgi arıyor',
  intentCommercial: 'Karşılaştırıyor',
  intentTransactional: 'Satın almaya hazır',
  intentNavigational: 'Markayı arıyor',

  stageAwareness: 'Sorunun farkında',
  stageConsideration: 'Seçenekleri tartıyor',
  stageDecision: 'Karar aşamasında',

  calendarAds: 'Yayındaki kampanyalar',
  calendarNoAds: 'Bu hafta yayında kampanya yok.',
  adsStartDate: 'Başlangıç tarihi',
  adsStartDateHint: 'Tarih girersen kampanya takvimde şerit olarak görünür.',
  adsSaveDate: 'Tarihi kaydet',
  adsNoStartDate: 'Tarih girilmedi',
};

export type Dictionary = typeof tr;
